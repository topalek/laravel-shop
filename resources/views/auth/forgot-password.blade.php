...
@section('content')
    <x-forms.auth-form title="Забыли пароль" action="{{route('forgot.password.email')}}" method="post">
        @csrf
        <x-forms.text-input
            name="email"
            type="email"
        ...
        <x-slot:buttons>
            <div class="flex justify-between mt-5">
                <div class="text-xxs md:text-xs">
                    <a class="text-white hover:text-white/70 font-bold" href="{{route('login')}}">Вспомнили пароль?</a>
                </div>
            </div>
